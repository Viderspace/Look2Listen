...
from avspeech.training.datasets import (
    CombinedAVDataset,
    OneSpeakerNoiseDataset,
    TwoSpeakerCleanDataset,
...
class MixedDataLoader:
    ...
    def __init__(
        ...
        """
        # Create datasets
        self.train_dataset: CombinedAVDataset = self._create_combined_dataset(
            train_paths
        )
        self.val_datasets: Dict[SampleT, Dataset] = self._create_separate_datasets(
            val_paths
        )
    ...
    def _create_separate_datasets(
        self, paths: Dict[SampleT, Path]
    ) -> Dict[SampleT, Dataset]:
        # Keep validation datasets separate for per-type evaluation
        val_datasets = {}
        if SampleT.S1_NOISE in paths:
            val_datasets[SampleT.S1_NOISE] = OneSpeakerNoiseDataset(
                paths[SampleT.S1_NOISE]
            )
        if SampleT.S2_CLEAN in paths:
            val_datasets[SampleT.S2_CLEAN] = TwoSpeakerCleanDataset(
                paths[SampleT.S2_CLEAN]
            )
        if SampleT.S2_NOISE in paths:
            val_datasets[SampleT.S2_NOISE] = TwoSpeakerNoiseDataset(
                paths[SampleT.S2_NOISE]
            )
        return val_datasets
    ...
    def get_train_loader(self):
        ...
        class BatchIterator:
            ...
            def __init__(
                self, dataset: CombinedAVDataset, sampler: SampleMixer, collate_fn
            ):
